/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { OperationLogWhereUniqueInput } from "./OperationLogWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { OperationLogUpdateInput } from "./OperationLogUpdateInput";

@ArgsType()
class UpdateOperationLogArgs {
  @ApiProperty({
    required: true,
    type: () => OperationLogWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => OperationLogWhereUniqueInput)
  @Field(() => OperationLogWhereUniqueInput, { nullable: false })
  where!: OperationLogWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => OperationLogUpdateInput,
  })
  @ValidateNested()
  @Type(() => OperationLogUpdateInput)
  @Field(() => OperationLogUpdateInput, { nullable: false })
  data!: OperationLogUpdateInput;
}

export { UpdateOperationLogArgs as UpdateOperationLogArgs };
